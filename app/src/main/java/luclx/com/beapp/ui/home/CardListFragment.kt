...
//				)
    ...
    private fun addRandomCard() {
        val name = (10000..100000).random().toString()
        Log.e("XXXX", "ADD $name")
        val newCard = createCard(name)
    ...
    private fun createCard(name: String) = CardEntity(
        first_name = name,
        address = "A",
        position = "P",
        gender = "G",
    ...
    private fun initNotifyData() {
        Handler().postDelayed({
            viewModel.notifyData().observe(this, Observer { card ->
                card?.let {
                    Log.e("XXXX", "NOTIFY " + card.first_name)
